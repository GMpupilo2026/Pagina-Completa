...
(function () {
  ...
  function makeGame(el, g) {
    ...
    function onHumanMove(uci) {
      if (state.mode === "adivinar") {
        ...
        if (ok || alt) {
          state.aciertos++; input.enable(false); state.ply = ply;
    ...
    function revealGuess(failed) {
      ...
      input.enable(false); state.ply = ply; renderMoves(); renderView();
    ...
    async function afterHumanPractice() {
      ...
      thinking = true; input.enable(false); msgEl.textContent = "El motor piensa…";
      let uci = null;
    ...
    function stopPractice() { state.mode = "ver"; input.enable(false); practBtn.textContent = "♟ Jugar desde aquí contra el motor"; guessBtn.disabled = false; msgEl.textContent = ""; renderMoves(); renderView(); }
  ...
  function makeExercise(el, x) {
    ...
    const input = boardInput(boardEl, promoEl, () => game, (uci) => {
      state.tries++;
