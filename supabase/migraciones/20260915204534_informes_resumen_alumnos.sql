-- Un renglón por alumno con TODO lo que la vista de resumen de informes.html
-- necesita ya contado. Antes esta misma cuenta se hacía en el navegador, que
-- para eso se bajaba question_answers, class_attendance, class_presence_log,
-- platform_activity_log y training_progress ENTERAS: a partir de las mil filas
-- PostgREST corta la respuesta sin avisar y los promedios salían calculados
-- sobre un pedazo de los datos, sin ningún error a la vista.
--
-- Es SECURITY INVOKER a propósito: quién ve a quién lo sigue decidiendo la RLS
-- de cada tabla, igual que cuando la consulta salía del navegador. Un profesor
-- recibe solo sus alumnos; quien administra, todos.
...
select a.id, a.full_name, a.email, a.grupo,
