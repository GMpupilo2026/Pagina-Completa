-- El reporte tiene que decir CUÁLES clases fueron presenciales.
--
-- Sin esto, la ficha de asistencia presencial entra al informe —eso ya lo hace
-- sola, porque es una class_sessions más— pero el informe la cuenta dentro de
-- «N clases en vivo», que es falso, y quien lo lee no tiene forma de separar
-- las de la plataforma de las del aula. Un informe que dice algo que no es se
-- ve exactamente igual de bien que uno que dice la verdad.
--
-- Lo único que cambia es que viaja la modalidad y que los totales la separan:
-- ningún número de los que ya había se calcula distinto.
